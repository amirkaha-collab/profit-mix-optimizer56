...
.compare-tbl td:nth-child(2) { text-align: right; font-size: 11px; color: #64748b; max-width: 160px; overflow: hidden; text-overflow: ellipsis; }
...
if active_tab == "📈 תוצאות":
    ...
    if res is None:
        st.info("עבור לטאב **הגדרות יעד** ולחץ **חשב חלופות**.")
    else:
        ...
        if not recs:
            ...
            for key, row, title in [
                ...
                ("service",  recs["service"],  "החלופה עם ציון השירות הגבוה ביותר"),
            ]:
                row = dict(row)
                row["חלופה"]        = title
                row["weights_items"] = _weights_items(row.get("weights"), row.get("קופות",""), row.get("מסלולים",""))
            ...
            # 8. Fourth card: best service (full width, styled differently)
            st.markdown("""
            <div style="border:2px solid #10b981;border-radius:14px;padding:6px 14px 2px;
                        background:linear-gradient(135deg,#f0fdf4,#ecfdf5);margin:4px 0 8px;">
              <span style="font-size:12px;font-weight:800;color:#065f46;">⭐ חלופה מיוחדת — עדיפות לאיכות שירות</span>
            </div>
            """, unsafe_allow_html=True)
            _render_reco_card(rows[3], "החלופה עם ציון השירות הגבוה ביותר", primary=False,
                              manager_names=manager_names, card_key='service', baseline=baseline)
            ...
            _safe_plotly(_radar_chart(top4, targets_used))

            # ── 9. Compact comparison table ──
            st.markdown("---")
            st.subheader("📊 טבלת השוואה")

            def _fmt_cell(val, is_pct=False):
                try:
                    v = float(val)
            ...
            header_row = "<tr><th>חלופה</th><th>מנהלים</th><th>משקלים</th><th>חו\"ל</th><th>מניות</th><th>מט\"ח</th><th>לא-סחיר</th><th>שארפ</th><th>שירות</th><th>Score</th></tr>"
            body_rows = ""
            for row in rows:
                body_rows += (
                    f"<tr>"
                    f"<td>{_esc(row.get('חלופה',''))}</td>"
                    f"<td style='font-size:11px;color:#64748b'>{_esc(str(row.get('מנהלים',''))[:40])}</td>"
                    f"<td>{_esc(row.get('משקלים',''))}</td>"
                    f"<td>{_fmt_cell(row.get('חו\"ל (%)'),True)}</td>"
                    f"<td>{_fmt_cell(row.get('מניות (%)'),True)}</td>"
                    f"<td>{_fmt_cell(row.get('מט\"ח (%)'),True)}</td>"
                    f"<td>{_fmt_cell(row.get('לא־סחיר (%)'),True)}</td>"
                    f"<td>{_fmt_cell(row.get('שארפ משוקלל'))}</td>"
                    f"<td>{_fmt_cell(row.get('שירות משוקלל'))}</td>"
                    f"<td style='font-weight:800;color:#1e3a8a'>{_fmt_cell(row.get('score'))}</td>"
                    f"</tr>"
                )
            st.markdown(f"""
